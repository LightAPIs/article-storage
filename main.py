...
def handler(path: str, is_new: bool, is_convert: bool, xlog: Logger):
    if os.path.exists(path):
        ...
        for each_name in all_list:
            ...
            if os.path.isdir(each_full_name):
                # 目录
                if not is_empty_dir(each_full_name) and not (
                    each_name in dir_ignore_list
                ):
                    ...
                    if not had:
                        if is_convert:
                            with io.open(each_full_name, mode="rb") as f:
                                ...
                                se = source_encoding["encoding"]
                                if se != "utf-8":
                                    xlog.info("转换文件编码格式: " + each_full_name)
                                    if se == "GB2312":
                                        content = data.decode("GBK")
                                        content = content.encode("utf-8").strip()
                                        codecs.open(each_full_name, "wb").write(
                                            str(content)
                                        )
                                    elif se:
                                        content = (
                                            data.decode(se).encode("utf-8").strip()
                                        )
                                        codecs.open(each_full_name, "wb").write(
                                            str(content)
                                        )
                        ...
                        with io.open(
                            each_full_name, mode="r", encoding="utf-8"
                        ) as file_load:
...
def is_empty_dir(dir_path):
    ...
    for name in os.listdir(dir_path):
        if is_txt_ext(name) or os.path.isdir(os.path.join(dir_path, name)):
            return False
    return True
